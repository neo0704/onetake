...
const css = `
  ...
  .ot-nav {
    ...
    background: rgba(8,8,16,0.88); -webkit-backdrop-filter:blur(20px); backdrop-filter:blur(20px);
  ...
  .ot-review-meta { font-size: 11px; color: rgba(255,255,255,0.55); }
  ...
  /* Lightbox */
  .ot-lightbox { position:fixed; inset:0; z-index:2000; background:rgba(0,0,0,0.95); display:flex; align-items:center; justify-content:center; -webkit-backdrop-filter:blur(8px); backdrop-filter:blur(8px); animation:fadeIn 0.2s ease; }
  ...
  .ot-drawer { position:fixed; top:68px; left:0; right:0; z-index:190; background:rgba(8,8,16,0.98); -webkit-backdrop-filter:blur(20px); backdrop-filter:blur(20px); border-bottom:1px solid var(--border); padding:8px 20px 20px; display:flex; flex-direction:column; animation:fadeIn 0.2s ease; }
  ...
  .ot-proj-card, .ot-proj-hero { -webkit-tap-highlight-color:transparent; }
...
export default function HomePage() {
  ...
  const modalRef     = useRef(null);   // project window (focus trap)
  const lbRef        = useRef(null);   // photo viewer (focus trap)
  const projOpener   = useRef(null);   // element to refocus when the project window closes
  const lbOpener     = useRef(null);   // element to refocus when the photo viewer closes
  ...
  useEffect(() => {
    const onKey = e => {
      // Keep Tab inside whichever window is open
      if (e.key === 'Tab') {
        const box = lightbox.open ? lbRef.current : activeProj ? modalRef.current : null;
        const items = box ? box.querySelectorAll('button, [role="button"], a[href]') : [];
        if (items.length) {
  ...
  // Give keyboard focus back to whatever opened the window / viewer
  useEffect(() => { if (!activeProj) return; return () => projOpener.current?.focus?.(); }, [activeProj]);
  useEffect(() => { if (!lightbox.open) return; return () => lbOpener.current?.focus?.(); }, [lightbox.open]);
  ...
  const openLightbox  = (images, idx) => { lbOpener.current = document.activeElement; setLightbox({ open: true, images, idx }); };
  ...
  const openProject = (proj) => { projOpener.current = document.activeElement; setActiveProj(proj); setVideoPlaying(false); };
  ...
  return (
    <div className="ot-root">
            ...
            {isDirectVideo(video.url) ? (
              ...
              <div className="ot-video-thumb" aria-label="Play featured video" {...press(() => setVideoPlaying(true))}>
                {getYTThumb(video.url)
                  ? <img src={getYTThumb(video.url)} alt="Video thumbnail" onError={e => { const el = e.currentTarget; el.onerror = null; el.src = el.src.replace('maxresdefault', 'hqdefault'); }} />
      ...
      {/* ── PROJECT MODAL ── */}
      {activeProj && (
        <div className="ot-modal-backdrop" ref={modalRef} role="dialog" aria-modal="true" aria-label={activeProj.title}>
      ...
      {/* LIGHTBOX */}
      {lightbox.open && (
        <div className="ot-lightbox" ref={lbRef} role="dialog" aria-modal="true" aria-label="Photo viewer"
          onClick={closeLightbox} onTouchStart={onLbTouchStart} onTouchEnd={onLbTouchEnd}>
          <button className="ot-lightbox-close" aria-label="Close photo" autoFocus onClick={closeLightbox}><IconClose /></button>
