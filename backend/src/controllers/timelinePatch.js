 ...
 *
 * In your quotation controller (e.g. sendQuotation / createQuotation), after setting
 * event.status = 'quotation_sent' and saving, add:
 *   event.addTimelineEntry('quotation_sent', req.user, '');
 *   await event.save();
 *   // OR if you use findByIdAndUpdate, push directly:
 *   await Event.findByIdAndUpdate(eventId, { $push: { timeline: {
 *     status: 'quotation_sent', label: 'Quotation Sent to Client',
 *     note: '', doneByName: req.user.name, completedAt: new Date(),
 *   }}});
 *
